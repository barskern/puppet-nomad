...
describe 'nomad' do
  ...
  context 'When passing a non-bool as purge_config_dir' do
    ...
    it { expect { is_expected.to compile }.to raise_error(%r{expects a Boolean value}) }
  end
  ...
  context 'When installing via URL by default' do
    it { is_expected.to contain_archive('/opt/puppet-archive/nomad-0.9.1.zip').with(source: 'https://releases.hashicorp.com/nomad/0.9.1/nomad_0.9.1_linux_amd64.zip') }
    it { is_expected.to contain_file('/opt/puppet-archive').with(ensure: 'directory') }
    it { is_expected.to contain_file('/opt/puppet-archive/nomad-0.9.1').with(ensure: 'directory') }
  ...
  context 'When installing via URL by with a custom url' do
    ...
    it { is_expected.to contain_archive('/opt/puppet-archive/nomad-0.2.3.zip').with(source: 'http://myurl0.2.3/nomad_0.2.3_linux_amd64.zip') }
    it { is_expected.to contain_file('/usr/local/bin/nomad').that_notifies('Class[nomad::run_service]') }
  end
